...
def snapshot_dng_metadata(path: Path) -> tuple[tuple[int, int, str, int, str], ...]:
    ...
    try:
        ...
        with tifffile.TiffFile(path) as tif:
            pages = _unique_pages(tif)
            return tuple(
                (int(page.offset), int(tag.code), str(tag.dtype), int(tag.count), repr(tag.value))
                for page in pages
...
def _unique_pages(tif) -> list:
    pages = {}
    for series in tif.series:
        for page in series.pages:
            pages[int(page.offset)] = page
    return [pages[offset] for offset in sorted(pages)]


def _find_tag(tif, preferred_page, code: int):
    tag = preferred_page.tags.get(code)
    if tag is not None:
        return tag
    for page in _unique_pages(tif):
...
def _numeric_tag(tag, label: str, count: int | None = None) -> np.ndarray:
    if tag is None:
        raise ExternalToolError(f"DNG 缺少 {label}")
    try:
        values = np.asarray(tag.value, dtype=np.float64).reshape(-1)
        if "RATIONAL" in str(tag.dtype).upper():
            if values.size != int(tag.count) * 2:
                raise ExternalToolError(f"DNG {label} 的有理数编码长度无效")
            pairs = values.reshape(-1, 2)
            if np.any(pairs[:, 1] == 0):
                raise ExternalToolError(f"DNG {label} 包含零分母")
...
def _validate_editability_tags(tif, page, metadata: RawMetadata, shape: tuple[int, ...]) -> None:
    repeat = tuple(int(v) for v in _numeric_tag(_find_tag(tif, page, 50713), "BlackLevelRepeatDim", 2))
    black = _numeric_tag(_find_tag(tif, page, 50714), "BlackLevel")
    if black.size == 4 and repeat != (2, 2):
        raise ExternalToolError("DNG BlackLevelRepeatDim 无法表示四位置黑电平")
    origin = _numeric_tag(_find_tag(tif, page, 50719), "DefaultCropOrigin", 2)
    size = _numeric_tag(_find_tag(tif, page, 50720), "DefaultCropSize", 2)
    ...
    matrix = _numeric_tag(_find_tag(tif, page, 50721), "ColorMatrix1", 9)
    if np.linalg.matrix_rank(matrix.reshape(3, 3)) < 3:
        raise ExternalToolError("DNG ColorMatrix1 不可逆")
    illuminant = _numeric_tag(_find_tag(tif, page, 50778), "CalibrationIlluminant1", 1)
    if illuminant[0] <= 0:
        raise ExternalToolError("DNG CalibrationIlluminant1 无效")
    neutral = _numeric_tag(_find_tag(tif, page, 50728), "AsShotNeutral", 3)
    if np.any(neutral <= 0):
        raise ExternalToolError("DNG AsShotNeutral 无效")
    orientation = _find_tag(tif, page, 274)
...
def _find_cfa_page(tif, metadata: RawMetadata, dtype: np.dtype) -> tuple[int, object, tuple[int, ...]]:
    candidates: list[tuple[int, object, tuple[int, ...]]] = []
    for series_index, series in enumerate(tif.series):
        ...
        if is_cfa and page.dtype == dtype and shape in {
            ...
            candidates.append((series_index, page, shape))
    if len(candidates) != 1:
...
def replace_cfa_pixels_in_place(path: Path, processed_visible: np.ndarray, metadata: RawMetadata) -> None:
    ...
    try:
        with tifffile.TiffFile(path) as tif:
            series_index, _, shape = _find_cfa_page(tif, metadata, processed_visible.dtype)
    ...
    try:
        mapped = tifffile.memmap(path, series=series_index, mode="r+")
    except Exception as exc:
...
def validate_processed_dng(path: Path, processed_visible: np.ndarray, metadata: RawMetadata) -> None:
    ...
    try:
        with tifffile.TiffFile(path) as tif:
            series_index, page, shape = _find_cfa_page(tif, metadata, processed_visible.dtype)
            _validate_editability_tags(tif, page, metadata, shape)
            ...
            for tag_code, expected, label in ((271, metadata.make, "Make"), (272, metadata.model, "Model")):
                tag = _find_tag(tif, page, tag_code)
            ...
            full = tifffile.memmap(path, series=series_index, mode="r")
            if shape == (metadata.height, metadata.width):
                visible = full
            else:
