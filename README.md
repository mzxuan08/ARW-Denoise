...
面向所有 Sony 2×2 Bayer ARW 相机的完整离线批量降噪软件。使用开源 PMRID 神经网络和 ONNX Runtime CUDA，输出保留 CFA、白平衡、相机型号、色彩元数据和传感器高光余量的可编辑 DNG。

## 使用发布包

1. 完整解压 `ArwDenoise-0.5.1-offline-win64.zip` 或体积更小的 `ArwDenoise-0.5.1-offline-win64-solid.7z`，不要只单独拖出 EXE。
...
0.4.0 移除 A7C II 型号白名单，改为 Sony 品牌与 RAW 能力检测。A1/A9/A7/A6xxx/A5xxx/NEX/FX/ZV/RX/SLT/DSLR 以及未来 Sony 型号使用同一条通用处理管线；只要 ARW 能可靠解码为 2×2 Bayer CFA，且位深、黑白电平和活动区域有效，即可处理。

0.5.0 在入队前后台并发预检兼容性，文件夹会递归发现子目录中的 ARW。队列显示相机型号、ISO、尺寸和位深；不兼容文件不会进入处理队列，并生成可定位的中文 JSON 报告。

0.5.1 修复 GUI 启动批处理时进度跟踪器参数错误导致所有任务立即失败的问题。0.5.0 中已经失败的任务会保留，升级后点击“重试失败任务”即可重新处理。

## 运行要求

- Windows 10/11 x64。
- 支持 CUDA 12 运行时的 NVIDIA 驱动。不需要安装 Python、CUDA Toolkit、cuDNN 或联网。
- 支持所有能通过能力检测的 Sony 2×2 Bayer ARW，不再按具体型号建立白名单。
- 当前实机回归机型为 Sony A7C II；其他 Sony 型号由 LibRaw/rawpy 与 dnglab 实际解析结果决定，不猜测缺失或矛盾的 RAW 元数据。
- 完整解压后约需 2.7 GB 空间。

PMRID 是通用 Bayer RAW 基线，不是 Sony 或 Adobe 的商业模型。首次使用新的相机型号时，请先抽取少量 DNG，在 Adobe Camera Raw/Lightroom 或像素蛋糕中确认你的实际工作流。
...
arw-denoise scan D:\SonyARW --workers 2 --report compatibility.json
```
